...
def store_file(data_dir, part_num, family_name):
    ...
    families = products[part_num]["family"]
...
def main():
    if len(sys.argv) > 2:
        ...
        if product_family:
            ...
            store_file(data_dir, part_number, product_family)
        else:
            print(f"Could not find product family for '{part_number}'.")
    else:
        print("please input a product number and data_dir")
